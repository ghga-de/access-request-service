...
```bash
docker pull ghga/access-request-service:5.0.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/access-request-service:5.0.0 .
```
...
docker run -p 8080:8080 ghga/access-request-service:5.0.0 --help
```
...
- <a id="properties/access_grant_max_extend"></a>**`access_grant_max_extend`** *(number)*: Max factor by which a data steward may extend the access grant. Minimum: `1`. Default: `5`.
