...
```bash
docker pull ghga/access-request-service:7.2.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/access-request-service:7.2.0 .
```
...
docker run -p 8080:8080 ghga/access-request-service:7.2.0 --help
```
